...
// ════════════════════════════════════════════════════════
// ПЕРЕМЕННЫЕ ОКРУЖЕНИЯ (должны быть заданы в Vercel)
// ════════════════════════════════════════════════════════
...
Если у видео отсутствуют какие-то метрики (написано "нет данных" или "недостаточно данных") — объясни, что для новых каналов или видео с малым количеством просмотров YouTube Analytics не предоставляет детальную статистику (обычно нужно 1000+ просмотров за 30 дней для retention/CTR метрик и определённый трафик для метрик Shorts). Не придумывай цифры которых нет.
...
async function upsertUserSafe(googleId, email, displayName, extraData={}) {
  try{
    ...
    if(error)console.error('Supabase upsert error (users):',error.message);
  }catch(e){ console.error('Supabase upsert exception (users):',e.message); }
}
...
/**
 * Гарантирует, что SUPREME_ADMIN_EMAIL присутствует в таблице admins
 * с is_primary=true и полными правами. Вызывается:
 *  - при каждом логине supreme-админа (callback)
 *  - при каждом обращении к /api/auth/me, если юзер совпадает с supreme email
 * Это устраняет ручную правку SQL при смене главного админа — следующий логин
 * новой supreme-почты сам создаст/обновит нужную запись.
 */
async function ensureSupremeAdminRecord(googleId, email) {
  if(email!==SUPREME_ADMIN_EMAIL)return;
  try{
    const existing=await getAdminRecord(email);
    if(!existing){
      const { error }=await supabase.from('admins').insert({
        user_id:googleId, email, is_primary:true, added_by:'system', permissions:ALL_PERMISSIONS,
      });
      if(error)console.error('ensureSupremeAdminRecord insert error:',error.message);
    }else if(!existing.is_primary || JSON.stringify(existing.permissions)!==JSON.stringify(ALL_PERMISSIONS)){
      const { error }=await supabase.from('admins').update({
        is_primary:true, permissions:ALL_PERMISSIONS, user_id:googleId||existing.user_id,
      }).eq('email',email);
      if(error)console.error('ensureSupremeAdminRecord update error:',error.message);
    }
  }catch(e){ console.error('ensureSupremeAdminRecord exception:',e.message); }
}
...
app.get('/api/auth/callback', async (req,res)=>{
  ...
  try{
    ...
    if(!tokenRes.ok||tokens.error){
      ...
      res.writeHead(302,{ Location:APP_URL+'/?error=token_exchange' }); return res.end();
    }
    ...
    catch(e){ console.error('Userinfo fetch failed:',e.message); res.writeHead(302,{ Location:APP_URL+'/?error=userinfo' }); return res.end(); }
    ...
    // Гарантируем supreme-админа в таблице admins без ручного SQL
    await ensureSupremeAdminRecord(googleId, email);
...
app.get('/api/auth/me', async (req,res)=>{
  ...
  // На каждый /me для supreme-почты также подтверждаем её права (защита от ручных правок в БД)
  await ensureSupremeAdminRecord(payload.sub, payload.email||'');
...
app.get('/api/youtube/videos', async (req,res)=>{
  ...
  try{
    ...
    res.json({
      ...
      analytics_limited: analyticsFailed || swipeFailed,
    });
  }catch(e){
...
app.get('/api/health', (req,res)=>{
  res.json({ ok:true, ts:new Date().toISOString(), version:'11.0.0', ai:'gemini-2.5-flash', supreme_admin:SUPREME_ADMIN_EMAIL, limits:{ user:USER_DAILY_LIMIT, admin:ADMIN_DAILY_LIMIT, feedback:FEEDBACK_DAILY_LIMIT } });
});
